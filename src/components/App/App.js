  ...
  function handleDeleteMovie(movie) {
    mainApi
      ...
      .then(() => {
        ...
        let prevFilterMovies = filterMovies.map((mov) =>
          mov.id === movie.movieId ? { ...mov, isSaved: false } : mov
        );
        console.log(prevFilterMovies);
        // Обновление состояния filterMovies после удаления фильма
        setFilterMovies(prevFilterMovies);

        // Используем актуальное значение prevFilterMovies для сохранения в локальном хранилище
        localStorage.setItem(
          "renderedMovies",
          JSON.stringify(prevFilterMovies)
        );
      })
      .catch((err) => console.log(err));
  }
      ...
      <div className="app">
        ...
        <Routes>
          ...
          <Route
            key="*"
            path="*"
            element={<PageNotFound handleGoBack={handleGoBack} />}
          />
          <Route
            key="signup"
            path="/signup/*"
